...
- **v1.5（2026-09-09，能力声明评审修正）**：
  - **D6 `off` 闸门**：`off` 不可用时本地报错，不再放行给 SDK 向上钳制（见 D6 v1.5）。
