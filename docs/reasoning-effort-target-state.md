...
| D6 | 服务器 400（取值不被模型接受）→ 按现状错误路径直接弹出（通知 + error block），不自动剥离参数重试。（v1.3 补充）**本地已声明不支持**（模型 `reasoning: false`，即 `getSupportedThinkingLevels` 仅含 `off`）却配置了具体档位 → 组请求时抛出**本地可见错误**（提示移除覆盖或在 `mutsumi.customProviders` 声明能力），不再依赖 SDK `clampThinkingLevel` 静默钳制丢弃。 |
