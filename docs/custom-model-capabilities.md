...
| # | 决策 |
|---|---|
| C1 | **乐观缺省**：自定义模型未声明能力时取 `reasoning: true`、`input: ['text', 'image']`。理由：能力未知 ≠ 不支持；选了档位而不兼容的端点会 400 可见（对齐 reasoning 文档 D6 哲学），用户可按 provider/模型手动禁用。（v1.4 补注）`input` 侧的代价是**降级行为翻转**：SDK 在请求构建期按 `model.input` 做图片降级（`transform-messages.downgradeUnsupportedImages`：无 image 支持时图片 part 被替换为 "(image omitted…)" 占位文本）。`['text']` 硬编码时期图片静默占位；乐观默认后图片真实上 wire，不支持视觉的端点 400 可见。依赖旧占位符行为的存量配置需显式声明 `input: ['text']`（已在 `CHANGELOG.md` 的 Unreleased 段点名）。 |
...
| C3 | **声明层级**（优先级从低到高）：乐观缺省 → provider 级 `capabilities` → `/models` 发现列表的数值字段（contextWindow/maxTokens）→ 模型级 spec。provider 级 `capabilities.reasoning: false` 与模型级 `reasoning: false` 都能关闭 reasoning；模型级 spec 是唯一能**覆盖 provider 级声明**的层。 |
| C4 | **声明存活于刷新**：SDK `createProvider` 合并语义是"动态发现结果按 id **覆盖**静态声明"（`dist/models.js` `currentModels()`），与 C3 优先级相反 → 合并必须在 `discoverCustomModels` 内部完成：发现项套用同 id 声明 spec 重建。声明独有 id 无需追加：`currentModels()` 是 baseline（含全部声明模型）与动态列表的并集，baseline 本身即携带声明。 |
