...
- **Breaking default flip for custom providers:** undeclared custom models are now
  ...
  image parts. If your endpoint is text-only, declare `input: ['text']` at the model
  or provider level to restore the previous behavior.
...
- **Visible error instead of silent drop:** setting a concrete reasoning effort on a
  ...
  explanatory error instead of having the effort silently clamped away.
